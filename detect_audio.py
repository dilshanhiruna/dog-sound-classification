...
class Monitor(object):
    def __init__(self, args = argparse.Namespace()):
        ...
        self.sample_size = int(self.seconds / self.dt)
    ...
    def read_chunk(self):
        ...
        return np.frombuffer(self.stream._stream.read(CHUNK), dtype=np.int16)            
    ...
    def wait_for_event(self, log=True):
        ...
        if log:
            ...
            # only if event is long than 3 seconds
            if self.detector.get_event_data().shape[0] > 2 * RATE:
                # save event as wav file
                self.save_event(self.detector.get_event_data())
    ...
    def save_event(self, data):
            ...
            # save inside records folder
            filename = f"records/event.wav"

            print(f"Saving event to {filename}")
